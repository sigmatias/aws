"""Ventana TXT siempre visible para las capturas EP3.

La ventana lee `informe/EP3_TXT_VISIBLE.txt` cada segundo y se mantiene
encima de las demas ventanas en el monitor secundario. Asi las capturas de
`snap_ep3_secondary.py` siempre incluyen los datos exigidos por la evaluacion.

Uso:
    python tools/ep3_evidence_overlay.py --write "Validacion AWS"
    python tools/ep3_evidence_overlay.py --show
"""
import argparse
import ctypes
import sys
...
TXT_PATH = ROOT / "informe" / "EP3_TXT_VISIBLE.txt"


BASE_LINES = [
    "Integrante: Matias Oyanedel",
    "Seccion: CUY1532-001D",
    "Asignatura: Comunicaciones Unificadas",
    "Evaluacion: EP3 CUY5132",
]
...
def show_overlay():
    ...
    root = tk.Tk()
    root.title("EP3_TXT_VISIBLE.txt")
...
def main():
    parser = argparse.ArgumentParser(description="TXT visible para evidencias EP3")
